...
export default function BrainGraph({ info }: { info: GenotypeInfo }) {
  ...
  for (const [partId, neurons] of byPart) {
    ...
    for (const neuron of neurons) {
      ...
      const color = funcColors[neuron.func] || "var(--color-border)";
        ...
        <g key={`n${partId}-${neuron.neuronId}`}>
          ...
          {neuron.inputs.map((inp, i) => (
            <text
              ...
              fontSize="9"
            >
              {"\u2190"} {inp.source.replace(/"/g, "")} (w:
              {inp.weight.toFixed(2)})
            </text>
          ))}
        </g>,
      );
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${svgW} ${y + pad}`}>
      {elements}
    </svg>
  );
}
